...
//	panic("mem_init: This function is not finished\n");
	...
	boot_map_region(kern_pgdir, (uintptr_t)UPAGES, size, PADDR(pages), PTE_U | PTE_P);
	...
	boot_map_region(kern_pgdir, (uintptr_t)UENVS, size, PADDR(envs), PTE_U | PTE_P);
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if(*pde & PTE_P) {
		...
		if(create && (pginfo = page_alloc(ALLOC_ZERO))!= NULL) {
			pginfo->pp_ref++;
			pgtab = (pte_t*)page2kva(pginfo);
			pgdir[PDX(va)] = PADDR(pgtab) | PTE_P | PTE_U | PTE_W;
			return &pgtab[PTX(va)];
		} else {
			return NULL;
		}	
...
page_insert(pde_t *pgdir, struct Page *pp, void *va, int perm)
{
	...
	if(pte == NULL) 
		return -E_NO_MEM;
	assert(pte != NULL);
	pp->pp_ref++; // should be there, can not put this operation after 
	// page_remove, because if put after page_remove, 
	// when the same pp is re_inserted, the pp maybe free in page_remove.
	// then we pp->ref++, so the the free pp have pp_ref > 0. this wrong.
	if(*pte | PTE_P)
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	// LAB 3: Your code here.
	uint8_t* low;
	assert((uintptr_t)va < (uintptr_t)va + len);
	uint8_t* upper = (uint8_t*)va + len;	
	low = (uint8_t*)ROUNDDOWN((uintptr_t)va, PGSIZE);
	upper = (uint8_t*)ROUNDUP((uintptr_t)upper, PGSIZE);
	pte_t *pte;
	perm = perm | PTE_P;
	for(; low < upper; low += PGSIZE){
		if((uintptr_t)low > ULIM) {
			user_mem_check_addr = (uintptr_t)va;
			return -E_FAULT;
		}
		pte = pgdir_walk(env->env_pgdir, low, 0);
		if(pte == NULL) {
			user_mem_check_addr = (uintptr_t)va;
			return -E_FAULT;
		}
